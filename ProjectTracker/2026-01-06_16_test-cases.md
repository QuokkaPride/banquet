# 2026-01-06_16_test-cases.md
# Step 16: Create Comprehensive Test File

## Metadata
- **Order**: 16 of 16
- **Estimated Time**: 10 minutes
- **Dependencies**: Step 15 (CSV updates)
- **Creates**: `test/smartOrdering.spec.ts`

## Purpose
Create test file that validates all smart ordering requirements against our 3 test patients.

---

## Prompt for LLM

Create `test/smartOrdering.spec.ts` with the following content:
```typescript
...
| Test | Patient | Validates |
|------|---------|-----------|
| Creates DINNER for Mark | Mark | Missing meal creation |
| Creates all 3 for Sophie | Sophie | Full day ordering |
| Creates LUNCH+DINNER for Alan | Alan | Partial day completion |
| No duplicates when run twice | All | Idempotency |
| No duplicate BREAKFAST for Mark | Mark | Existing order respected |
| Never creates SNACK | All | SNACK exclusion |
| Sophie gets meals without diet order | Sophie | Default calorie handling |
| Mark within 1500-2500 | Mark | Diet order compliance |
| Alan reaches 2000+ | Alan | Calorie catch-up |
| Each meal has entree | All | Meal composition |
| No breakfast items in dinner | All | Meal time appropriateness |
| No coffee in dinner | All | Meal time appropriateness |
...
# Run only smart ordering tests
npm test -- --testPathPattern=smartOrdering
```

---

## Validation Checklist
- [ ] Test file created at `test/smartOrdering.spec.ts`
- [ ] `npm test` runs without syntax errors
- [ ] Tests are pending (will fail until implementation complete)